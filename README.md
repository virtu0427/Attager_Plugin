...
### 샘플 데이터
`docker compose up` 시 `redis-agents-seeder` 서비스가 `agentDB/seed_data.txt`에 정의된 샘플 데이터를 에이전트용 Redis(`redis-agents`)에 자동으로 적재합니다. 이미 시드가 존재하면 재실행 시에도 중복 적재를 건너뜁니다.
