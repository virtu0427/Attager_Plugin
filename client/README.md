...
- 클라이언트 UI: http://localhost:8010/login (로그인 성공 후 `/chat` 진입)
...
- `/api/chat` → `message/send` JSON-RPC 요청을 작성해 오케스트레이터로 전달 (FastAPI에서 세션이 없으면 즉시 401 반환)
  - 세션에 저장된 JWT를 `Authorization: Bearer ...` 헤더와 `X-User-Email` 헤더로 함께 보내므로, 오케스트레이터/에이전트 플러그인 쪽에서 사용자 토큰을 활용해 도구 사용을 제한할 수 있습니다.
